...
public class SynQGService {
    ...
    public List<GeneratedQuestion> generateQuestionAnswers(List<String> inputs) {
        ...
        if (enableBackTranslation) {
            ...
            // java chunk split generatedQuestions
            // into chunks of size 100
            List<List<GeneratedQuestion>> generatedQuestionsChunks = Lists.partition(generatedQuestions, 100);

            generatedQuestionsChunks.forEach(generatedQuestionChunk -> {
                // cat each sentence with newline
                StringBuilder sb = new StringBuilder();
                generatedQuestionChunk.forEach(generatedQuestion -> {sb.append(generatedQuestion.question()).append("\n");});
                // get string from sb
                String generatedQuestionsString = sb.toString();
                generatedQuestionsString = generatedQuestionsString.trim();

                try {
                    String question = backTranslationService.backTranslate(generatedQuestionsString);
                    
                    // split the question into individual questions
                    String[] questions = question.split("\n");
                    for (int i = 0; i < questions.length; i++) {
                        String q = questions[i];
                        GeneratedQuestion generatedQuestion = generatedQuestionChunk.get(i);
                        generatedQuestion.question(q);
                    }
                } catch (Exception e) {
                    e.printStackTrace();
                }
